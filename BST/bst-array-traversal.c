...
void postorder(int* root,int idx)
{
  if(root[idx]!=-1)
  {
    int tmp = idx<<1;
    postorder(root,tmp);
    postorder(root,tmp+1);
    printf("%d ",root[idx]);
  }
}
...
int main(void)
{
  ...
  printf("inorder : ");inorder(Tree,1);puts("");
  printf("preorder : ");preorder(Tree,1);puts("");
  printf("postorder :"); postorder(Tree,1);puts("");
